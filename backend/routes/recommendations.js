...
// // router.post('/:sessionId/generate', authenticate, async (req, res) => {
// //   res.json(
// //     formatResponse(false, null, 'Recommendations coming soon - AI services need to be configured')
// //   );
// // });
...
import { authenticate } from '../middleware/auth.js';
import { formatResponse } from '../utils/helpers.js';
import StyleSession from '../models/StyleSession.js';  // ← DEFAULT IMPORT (no curly braces)
import User from '../models/User.js';
...
router.post('/:sessionId/generate', authenticate, async (req, res) => {
  try {
    ...
    // TODO: Add AI recommendation generation here
    res.json(
      formatResponse(false, null, 'AI recommendation service not yet configured. Add your OpenAI/Anthropic API keys to enable this feature.')
    );
...
router.get('/saved', authenticate, async (req, res) => {
  try {
    ...
    const sessions = await StyleSession.find({
      userId,
      'userInteraction.saved': true,
      recommendations: { $exists: true }
    })
...
router.post('/:sessionId/regenerate/:category', authenticate, async (req, res) => {
  try {
    ...
    // TODO: Add AI regeneration logic here
    res.json(
      formatResponse(false, null, 'AI regeneration service not yet configured')
    );
